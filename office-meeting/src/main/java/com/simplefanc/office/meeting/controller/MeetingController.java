...
import com.simplefanc.office.meeting.controller.form.*;
...
public class MeetingController {
    ...
    public R insertMeeting(@Valid @RequestBody InsertMeetingForm form, @RequestHeader("token") String token) {
        if (form.getType() == 2 && StrUtil.isBlank(form.getPlace())) {
    ...
    public R searchRoomIdByUUID(@Valid @RequestBody SearchRoomIdByUUIDForm form,
        ...
        int userId = jwtUtil.getUserId(token);
        UserDTO tbUser = userFeignClient.searchById(userId);
//        UserDTO tbUser = (UserDTO) map.get("result");
        int roomId = meetingService.searchRoomIdByUUID(form.getUuid());
        return R.ok().put("result", roomId).put("userName", tbUser.getName());
    }
